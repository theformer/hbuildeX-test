<template>
	<view class="schedule-details">
		...
		<view class="" v-show="current==1">
			<u-collapse class="collapse-conteinar"   :arrow="arrow" :accordion="accordion" ref="collapse">
				<u-collapse-item class="collapseName"  v-for="(itemVideo,indexVideo) in videoTeachList" :key="indexVideo" @change="collapseClick" :index="indexVideo">
					<template slot="title" style="background:rgba(244,247,252,1);">
						<view class="" style="display: flex;flex-direction: row; margin-left: 30rpx;">
							<image src="/static/my/subtract.png" style="width: 26rpx;height: 26rpx;" mode="" v-if="itemVideo.flag==true"></image>
							<image src="/static/my/sum.png" style="width: 26rpx;height: 26rpx;"  mode="" v-if="itemVideo.flag!=true"></image>
					...
					<view class="footer" v-for="(itemChapterVideos,indexChapterVideos) in itemVideo.videoList" :key="indexChapterVideos">
							...
							<view class="footer-time">
								...
								<view class="">
									<text class="min">{{itemChapterVideos.totalTime||0}}</text>分钟
								</view>
							</view>
					</view>
				</u-collapse-item>
	...
	export default{
		...
		methods:{
			...
			linkToCourse:function(itemChapterVideos){
				...
				setTimeout(function(){
					uni.navigateTo({
						url:'../live/recorded-videos?subCourse='+itemChapterVideos.id+'&subCourseId='+itemChapterVideos.subCourseId,
			...
			collapseClick:function(e){
				...
				if(e.show ==true){
					...
					this.videoTeachList[e.index].flag = true
				}else{
					this.videoTeachList[e.index].flag = false
				}
			},
