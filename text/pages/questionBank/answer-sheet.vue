	...
	<view class="answer-sheet">
				...
				<swiper :indicator-dots="false" :autoplay="false" :current="questionIndex" @change="changeSwiper" :style="{'height':swiperHeight}">
					<swiper-item v-for="(item,index) in questionList" :key="index" class="swiper-item2">
						...
						<view v-if="!paperId&&!chapterId">
							...
							<radio-group @change="radioboxChange"  v-if="item.questionTypeId ==1">
								<view class="answer-style">
									<radio class="radio-class"  value="A" :checked="item.examObAnswer!=undefined&&item.examObAnswer=='A'" color="#1d2087" />
									<text class="choose-answer">A.</text>
									<view class="text-class" >{{item.a}}</view>
								</view>
								<view class="answer-style">
									<radio  class="radio-class" value="B" :checked="item.examObAnswer!=undefined&&item.examObAnswer=='B'" color="#1d2087" />
									<text class="choose-answer">B.</text>
									<view class="text-class" >{{item.b}}</view>
								</view>
								<view class="answer-style">
									<radio  class="radio-class" value="C" :checked="item.examObAnswer!=undefined&&item.examObAnswer=='C'" color="#1d2087"/>
									<text class="choose-answer">C.</text>
									<view class="text-class" >{{item.c}}</view>
								</view>
								<view class="answer-style" v-if="item.d!==''">
									<radio  class="radio-class" value="D" :checked="item.examObAnswer!=undefined&&item.examObAnswer=='D'" color="#1d2087"/>
							...
							<checkbox-group    @change="checkboxChange" v-if="item.questionTypeId ==2">
								<view class="answer-style">
									<checkbox  class="radio-class checkbox" :checked="item.infoA" value="A"  color="#1d2087"/>
									<text class="choose-answer">A.</text>						
									<view class="text-class" >{{item.a}}</view>
								</view>
								<view class="answer-style">
									<checkbox class="radio-class checkbox"  :checked="item.infoB" value="B"   color="#1d2087"/>
									<text class="choose-answer">B.</text>
									<view class="text-class" >{{item.b}}</view>
								</view>
								<view class="answer-style">
									<checkbox class="radio-class checkbox"  :checked="item.infoC" value="C"   color="#1d2087"/>
									<text class="choose-answer">C.</text>
									<view class="text-class" >{{item.c}}</view>
								</view>
								<view class="answer-style" v-if="item.d!==''">
									<checkbox class="radio-class checkbox"  :checked="item.infoA" value="D"   color="#1d2087"/>
									<text class="choose-answer">D.</text>
									<view class="text-class" >{{item.d}}</view>
								</view>
							</checkbox-group>
							...
							<radio-group @change="radioboxChange" v-if="item.questionTypeId ==3" >
								<view class="answer-style">
									<radio  class="radio-class" value="A" color="#1d2087" :checked="item.examObAnswer!=undefined&&item.examObAnswer=='A'" />
									<text class="choose-answer">A.</text>
									<view class="text-class" >{{item.a}}</view>
								</view>
								<view class="answer-style">
						...
						<checkbox-group    @change="checkboxChange" v-if="item.questionTypeId ==3">
							...
							<!-- 简答题 -->
							<view v-if="item.questionTypeId ==4&&!clear">
								<textarea style="height:245rpx;" class="textarea-style" @input="textBlurClick" :value="item.examObAnswer" type="textarea" maxlength="-1" :disabled="activeDisabled" :auto-height="true"></textarea>
							</view>
						...
						<view  class="margin-top" v-show="!seeOK||isShowOk">
							<view class="cu-bar">
								...
								<view class="action  text-grey">
									<text>我的选择：</text>
									<text class="text-red">{{item.examObAnswer||'未作答'}}</text>
								</view>
							</view>
							<view class="cu-bar-title">
								<view class="action  text-grey">
									<image src="../../static/questionBank/组 6.png" class="answer-png"></image>
								</view>
							</view>
		...
		<view class="footer" id="foot-box">
			<view class="tabbar-style" v-if="seeOK||!isShowOk" @click="saveClick">
				<image src="../../static/questionBank/交卷02.png" class="footer-png"></image>
				<text class="footer-text">交卷</text>
			</view>
			<view class="tabbar-style" v-if="clear" @click="clearClick">
				<image src="../../static/questionBank/垃圾桶.png" class="footer-png"></image>
				<text class="footer-text">移除</text>
			</view>
			<view class="tabbar-style" @click="finishClick">
				<image src="../../static/questionBank/纠错.png" class="footer-png"></image>
				<text class="footer-text">纠错</text>
			</view>	
			<view class="tabbar-style-right" @click="answerSheetClick">
				<image src="../../static/questionBank/答题卡(1).png" class="footer-png-right"></image>
		...
		<u-modal 
			...
			<view class="modal">
				<view class="modal-head">
					<image class="modal-png" src="../../static/questionBank/组 8.png"></image>
				</view>
	...
	export default{
		...
		methods:{
			...
			clearClick:function(e){
				...
				uni.request({
					...
					success: (res) => {
						if (res.data.code ==200) { //res.data.data为空时请求数据失败
							uni.showToast({
								icon: 'none',
		...
		onLoad:function(option){
			...
			if(this.isShow ==1){
				...
				this.clear = true
				console.log(this.isShow,this.seeAnswer,!this.clear)
			}
			if(this.seeAnswer ==1 ){
				this.activeDisabled = true
				this.seeOK = false
