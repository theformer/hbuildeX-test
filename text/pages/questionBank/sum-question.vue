		...
		<view class="demo" :style="'height:' + 260 + 'rpx;' + 'padding-top:' + demo.top + 'px;padding-bottom:10rpx'">
		   <view class="left" :style="'top:' + 20 + 'px'">
			   <view class="iconfont icon-xiaoxi">
						<image src="../../static/my/返回.png" @click="backToSuperior" class="back-btn" mode=""></image>	
						<view class="title">
	...
	export default{
		...
		methods:{
			...
			clearCourse:function(e){
				this.courseList.push({name:e.value.name})
			...
			// //点击行，会触发onclick事件，event = {index,value},index-被点击行下标，value-被点击行数据。
			...
			// //清空课程
			// clearSubCourse:function(item){
			// 	this.courseList.push({name:item.name})
			// 	for(let i =0;i<this.diaCourse.length;i++){
			// 		if(item.name ==this.diaCourse[i].name){
			// 			this.diaCourse.splice(i,1)
			// 		}
			// 	}
			// },
